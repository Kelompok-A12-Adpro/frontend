...
import { Wallet, TrendingUp, TrendingDown, Smartphone, ArrowRight } from "lucide-react"
  ...
  return (
    <div className="flex items-center justify-between p-4 border-b border-neutral-100 last:border-b-0 hover:bg-neutral-50/50 transition-colors duration-200">
      <div className="flex items-center gap-3">
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center ${
            isTopup ? "bg-green-100 text-green-600" : "bg-pink-100 text-pink-600"
          }`}
        >
          {isTopup ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
        </div>
        <div>
          <div className="font-medium text-neutral-900 capitalize">{transaction.transaction_type}</div>
          <div className="text-sm text-neutral-500">
            {transaction.payment_method && `via ${transaction.payment_method}`}
            {transaction.campaign_title && `to ${transaction.campaign_title}`}
          </div>
          {transaction.message && <div className="text-xs text-neutral-400 italic mt-1">"{transaction.message}"</div>}
      ...
      <div className="text-right">
        <div className={`font-bold ${isTopup ? "text-green-600" : "text-pink-600"}`}>
          {isTopup ? "+" : "-"}
          {formatCurrency(transaction.amount)}
        </div>
        <div className="text-xs text-neutral-500 capitalize mb-2">{transaction.status}</div>
        <Button
          variant="ghost"
          size="sm"
...
export default function WalletPage() {
  ...
  const loadWalletData = async () => {
    try {
      ...
      const [walletData, transactionData] = await Promise.all([fetchWallet(), fetchTransactions()])
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg text-red-800">
            {error}
            <Button variant="ghost" onClick={loadWalletData} className="ml-3 text-red-600 hover:text-red-700 underline">
              Retry
            </Button>
          </div>
        )}
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-neutral-600 max-w-2xl mx-auto">
            Manage your donations and track your impact with our secure wallet system
          </p>
        </div>
        ...
        {activeTab === "wallet" && wallet && (
          <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button onClick={() => setActiveTab("topup")} className="inline-flex items-center gap-2">
                Top Up Wallet
            ...
            <Card className="bg-white/80 backdrop-blur-sm">
              ...
              <CardContent className="space-y-6">
                <form onSubmit={handleTopUp} className="space-y-6">
                  {/* Amount Selection */}
                  <div>
                    <label className="block text-sm font-medium text-neutral-700 mb-3">Amount (Rp)</label>
                    <input
                      type="number"
                  ...
                  {/* Phone Number */}
                  <div>
                    <label className="block text-sm font-medium text-neutral-700 mb-3">Phone Number</label>
                    <input
                      type="tel"
              ...
              <CardContent className="p-0">
                {transactions.length > 0 ? (
                  ...
                  <div className="text-center py-12 text-neutral-500">
                    ...
                    <p>Start by topping up your wallet to see transactions here</p>
                  </div>
                )}
              </CardContent>
            </Card>
